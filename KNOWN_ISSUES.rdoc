...
After BioRuby 1.4.1, when running on Ruby 1.9.X, escaping of command-line
arguments are changed to be processed by the Ruby interpreter. Before BioRuby
1.4.0, the escaping is executed in Bio::Command#escape_shell_windows, and the
behavior is different from the Ruby interpreter's. Due to the change,
