#
# bio/io/pubmed.rb - NCBI PubMed client class
#
...
#   path = "/entrez/utils/pmfetch.fcgi?db=PubMed&mode=text&report=medline&id="
#   path = "/entrez/query.fcgi?cmd=Text&db=PubMed&dopt=MEDLINE&uid="
    path = "/entrez/query.fcgi?cmd=Search&db=PubMed&doptcmdl=MEDLINE&term="
    http = Net::HTTP.new(host)

    @pubmed	= Hash.new('')
    result	= http.get(path + id).pop
