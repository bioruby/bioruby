...
module Bio
  class TestCodonTableConstants < Test::Unit::TestCase
    ...
    def test_Starts
      ...
      assert_equal(%w[ttg ctg atg gtg], Bio::CodonTable::STARTS[1])
    end

    def test_stops
      assert(Bio::CodonTable::STOPS)
      assert_equal(%w[taa tag tga], Bio::CodonTable::STOPS[1])
    end
  ...
  class TestCodonTable < Test::Unit::TestCase
    ...
    def test_start
      assert_equal(%w[ttg ctg atg gtg], @ct.start)
    end

    def test_stop
      assert_equal(%w[taa tag tga], @ct.stop)
    end
