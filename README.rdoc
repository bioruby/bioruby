...
doc/Changes-0.7.md:: News and incompatible changes from 0.6.4 to 1.2.1.
...
When the {simplecov}[https://rubygems.org/gems/simplecov] gem is installed
(it is included in the development group of the Gemfile), running the
self-tests automatically measures code coverage.  Install the development
gems and run the tests:

  % bundle install
  % bundle exec rake

When the tests finish, a short summary is printed and a coverage report is
generated in the coverage/ directory:

  Coverage report generated for ... to coverage/index.html
  Line coverage: 13943 / 16687 (83.55%)
...
The machine-readable results are also written to coverage/.resultset.json.
If simplecov is not installed, the tests run normally without coverage.
