...
If possible, it may be recommended to install the following plugins:
* bio-shell -- if you use the BioRuby Shell.
* bio-executables -- if you use br_bio* commands.
* bio-old-biofetch-emulator -- if you run existing codes using BioFetch, including sample codes in demo/.
* bio-blast-xmlparser -- if Expat XML parser (with development files) is installed in your system.
